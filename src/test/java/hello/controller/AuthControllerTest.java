...
class AuthControllerTest {
    ...
    void testLogin() throws Exception {
        ...
        mvc.perform(get("/auth"))
                .andExpect(status().isOk())
                .andExpect(result -> Assertions.assertTrue(result.getResponse().getContentAsString(Charset.forName("UTF8")).contains("用户没有登录")));
        ...
        MvcResult response = mvc.perform(post("/auth/login").contentType(MediaType.APPLICATION_JSON_UTF8)
                ...
                .andExpect(status().isOk())
                .andExpect(result -> Assertions.assertTrue(result.getResponse().getContentAsString(Charset.forName("UTF8")).contains("登录成功")))
                .andReturn();
