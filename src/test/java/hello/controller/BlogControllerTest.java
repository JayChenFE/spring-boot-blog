...
class BlogControllerTest {
    ...
    void requireLoginBeforeProceeding() throws Exception {
        mvc.perform(post("/blog").contentType(MediaType.APPLICATION_JSON_UTF8)
                ...
                .andExpect(result -> Assertions.assertTrue(result.getResponse().getContentAsString(Charset.forName("UTF8")).contains("登录后才能操作")));
    }

    @Test
    void invalidRequestIfTitleIsEmpty() throws Exception {
        Mockito.when(authService.getCurrentUser()).thenReturn(Optional.of(new User(1L, "mockUser", "")));
        mvc.perform(post("/blog").contentType(MediaType.APPLICATION_JSON_UTF8)
                .content("{\"content\":\"123\"}"))
                .andExpect(status().isOk())
